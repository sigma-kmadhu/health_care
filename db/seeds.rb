...
company_list.each_with_index do |company, index|
    ...
    company_obj.users.create({email: "#{company.parameterize}@gmail.com", password: 'password', password_confirmation: 'password', actual_company_id: actual_company_id})
    60.times do |index|
        actual_patient_id = 5000 + index
        patient_obj = company_obj.patients.create(
            ...
            loc: "LOC_#{index}", actual_patient_id: actual_patient_id, actual_company_id: actual_company_id)
        7.times do |dw|
            patient_obj.daywise_infos.create(t_date: Date.today - (dw+1).day, status: "PHP", actual_patient_id: actual_patient_id)
        end
    end
end
